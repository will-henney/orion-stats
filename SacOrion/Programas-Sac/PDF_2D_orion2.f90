program  PDF_2D
  ...
  logical, dimension(:,:), allocatable :: vel_mask
 ...
 ! print*,'filename ', trim(emtype)//'_-040+070.wisomom-sum-smooth2d.fits'
!  call fitsread(trim(emtype)//'_-040+070.wisomom-sum-smooth2d.fits') 
!  nx = size(fitsimage, 1) 
!  ny = size(fitsimage, 2)
...
!  m0=fitsimage
  ...
  allocate(c(nbin), vel_mask(nx, ny) ) 
  c=0                  
...
!vel_mask= (Vc>-50.and.Vc<100)

  sumv = 0.0
  npts = 0
  do i=4,302
     do j=80,550
           if (Vc(i,j)>-50.and.Vc(i,j)<100)then      
           sumv = sumv + Vc(i,j)
           npts = npts + 1
           end if 
...
archivos=trim(emtype)//'-pdf-vc3'//'.dat'
